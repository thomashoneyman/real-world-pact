...
const Markets = () => {
  ...
  const Market = ({ asset }: { asset: AssetName }) => (
    <MarketCard
      ...
      supplyRate={supplyInterestRates.get(asset)}
      borrowRate={borrowInterestRates.get(asset)}
    />
  );
...
interface MarketCardProps {
  asset: AssetName;
  price: undefined | RequestStatus<number>;
  market: undefined | RequestStatus<MarketState>;
  supplyRate: undefined | RequestStatus<number>;
  borrowRate: undefined | RequestStatus<number>;
}
      ...
      >
        ...
        >
          <Flex css={{ display: "flex", flexDirection: "column", alignSelf: "center " }}>
            ...
            <RequestLoader
              request={price}
              success={(parsed) => <Text>${parsed.toFixed(2)}</Text>}
            />
          </Flex>
          <Flex css={{ display: "flex", flexDirection: "column", alignSelf: "center " }}>
            <Label>Supply</Label>
            <RequestLoader
              request={market}
              success={({ totalSupply }) => <Text>{totalSupply.toFixed(2)}</Text>}
            />
          </Flex>
          <Flex css={{ display: "flex", flexDirection: "column", alignSelf: "center " }}>
            ...
            <RequestLoader
              request={supplyRate}
              success={(rate) => <Text>{(rate * 100.0).toFixed(2)}%</Text>}
            />
          </Flex>
          <Flex css={{ display: "flex", flexDirection: "column", alignSelf: "center " }}>
            <Label>Borrows</Label>
            <RequestLoader
              request={market}
              success={({ totalBorrows }) => <Text>{totalBorrows.toFixed(2)}</Text>}
            />
          </Flex>
          <Flex css={{ display: "flex", flexDirection: "column", alignSelf: "center " }}>
...
const UserCard = () => {
  ...
  const Borrow = ({ asset }: { asset: AssetName }) => (
    <Box>
      <Label>{asset}</Label>
      <RequestLoader
        request={user.borrows.get(assetToToken(asset))}
        success={(parsed) => <Text>{parsed.toFixed(2)}</Text>}
      />
    </Box>
  );
  ...
  const Supply = ({ asset }: { asset: AssetName }) => (
    <Box>
      <Label>{asset}</Label>
      <RequestLoader
        request={user.supply.get(assetToToken(asset))}
        success={(parsed) => <Text>{parsed.toFixed(2)}</Text>}
      />
    </Box>
  );
