; The market for KETH with the cwKETH interest token. See the 'market-interface'
...
(module cwKETH GOVERNANCE
  @doc "The Charkha token for the KETH money market."
  ...
  (defpact transfer-crosschain:string (sender:string receiver:string receiver-guard:guard target-chain:string amount:decimal)
    (step (format "{}" [(enforce false "Cross-chain transfers not supported for cwKETH.")])))
...
(if (read-msg "init")
  [ (create-table free.cwKETH.participants-table)
    (create-table free.cwKETH.refs-table)
  ]
  "Upgrade complete")
