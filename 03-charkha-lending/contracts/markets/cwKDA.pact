; The market for KDA with the cwKDA interest token. See the 'market-interface'
...
(module cwKDA GOVERNANCE
  @doc "The Charkha token for the KDA money market."
  ...
  (defpact transfer-crosschain:string (sender:string receiver:string receiver-guard:guard target-chain:string amount:decimal)
    (step (format "{}" [(enforce false "Cross-chain transfers not supported for cwKDA.")])))
...
(if (read-msg "init")
  [ (create-table free.cwKDA.participants-table)
    (create-table free.cwKDA.refs-table)
  ]
  "Upgrade complete")
